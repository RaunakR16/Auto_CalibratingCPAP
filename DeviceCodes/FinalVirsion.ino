...
#define RELAY2_PIN       4      // ← Secondary relay (same 1s ON / 30s OFF cycle)
...
#define BASELINE_WINDOW_MS   90000UL      // 1.5-minute baseline collection
...
// ═══════════════════════════════════════════════ HUMIDIFIER CONFIG
static const unsigned long HUM_ON_TIME        = 1000UL;   // 1 s ON
static const unsigned long HUM_OFF_TIME       = 30000UL;  // 30 s OFF
...
// ═══════════════════════════════════════════════ BPM DISPLAY REFRESH
// Refreshes currentBPM on a timer so the OLED stays live between breath peaks.
// Works in both AUTO and MANUAL modes — no waiting, no gating on mode.
...
void setup()
{
    ...
    // ── Sensor calibration + pressure tare
    calibrateSensor();
    tareHX710B();
...
void loop()
{
    ...
    // ── Periodic BPM refresh — keeps display live between peaks in both AUTO & MANUAL
...
bool detectBreath(int filtered, unsigned long nowMs)
{
    ...
    else if (inPeak && filtered < exit)
    {
        ...
        if ((nowMs - lastPeakMs) >= BREATH_MIN_GAP_MS)
        {
            ...
            unsigned long winStart = (nowMs > LIVE_BPM_WINDOW_MS)
                                     ? (nowMs - LIVE_BPM_WINDOW_MS) : 0;
            computeStats(winStart, &currentBPM,
...
void computeStats(unsigned long windowStartMs, float* bpm, float* avg, float* mn, float* mx)
{
    ...
    if (cnt < 2) { *bpm = 0; *avg = 0; *mn = 0; *mx = 0; return; }
...
void runAutoLogic(unsigned long nowMs)
{
    ...
    case AUTO_COLLECT:
    {
        ...
        {
            computeStats(autoPhaseStartMs,
                         &baselineBPM, &avgInterval_s, &minInterval_s, &maxInterval_s);
            currentBPM = baselineBPM;

            printStatsSerial("BASELINE RESULT",
                             baselineBPM, avgInterval_s, minInterval_s, maxInterval_s);
    ...
    case AUTO_WATCH:
    {
        ...
        if (baselineBPM > 0.0f && currentBPM > 0.0f && currentBPM < (baselineBPM - BPM_HYSTERESIS))
        {
            ...
            Serial.printf(
                "[AUTO] BPM dropped to %.1f (< threshold %.1f) — THERAPY START, Level %d\n",
    ...
    case AUTO_THERAPY:
    {
        if (nowMs - lastTherapyCycleMs >= THERAPY_CYCLE_MS)
        {
            ...
            unsigned long cycleStart = (nowMs > THERAPY_CYCLE_MS)
                                       ? (nowMs - THERAPY_CYCLE_MS) : 0;
...
void calibrateSensor()
{
    ...
    long sum = 0;
    for (int i = 0; i < 50; i++) { sum += analogRead(SENSOR_PIN); delay(60); }
...
// ══════════════════════════════════════════════ SERIAL STATS PRINT
void printStatsSerial(const char* label,
                      float bpm, float avg, float mn, float mx)
{
    Serial.println("─────────────────────────────────────────────");
    Serial.printf("  [%s]\n", label);
...
void rampToSpeed(int target)
{
    ...
    if (currentSpeed < target)
        for (int s = currentSpeed; s <= target; s += RAMP_STEP)
            { esc.writeMicroseconds(s); delay(RAMP_DELAY); }
    else
        for (int s = currentSpeed; s >= target; s -= RAMP_STEP)
            { esc.writeMicroseconds(s); delay(RAMP_DELAY); }
...
long readHX710B_raw()
{
    ...
    long data = 0;
    for (int i = 0; i < 24; i++) {
        digitalWrite(HX710B_SCK, HIGH); delayMicroseconds(2);
        int bit = digitalRead(HX710B_DOUT);
        digitalWrite(HX710B_SCK, LOW);  delayMicroseconds(2);
        data = (data << 1) | bit;
    }
    digitalWrite(HX710B_SCK, HIGH); delayMicroseconds(2);
    digitalWrite(HX710B_SCK, LOW);  delayMicroseconds(2);
...
void tareHX710B()
{
    ...
    for (int i = 0; i < 3; i++) { readHX710B_raw(); delay(100); }

    long sum = 0;
    for (int i = 0; i < 20; i++) { sum += readHX710B_raw(); delay(100); }
